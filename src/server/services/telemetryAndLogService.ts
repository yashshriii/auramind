...
// Bounded capacity limits for persistent store
const MAX_SESSIONS = 5000;
const MAX_LOGS = 2000;
const MAX_QA_PER_SESSION = 100;

const DATA_DIR = path.join(process.cwd(), 'data');
const SESSIONS_FILE = path.join(DATA_DIR, 'telemetry_sessions.json');
const LOGS_FILE = path.join(DATA_DIR, 'system_logs.json');

const sessionsMap = new Map<string, SessionTelemetry>();
const systemLogs: SystemLog[] = [];

// Initialize & Hydrate from Disk
function initializePersistentStore(): void {
  try {
    ...
    console.error('Error hydrating telemetry store from disk:', err);
  }
}
...
function persistLogsDebounced(): void {
  ...
  logSaveTimeout = setTimeout(() => {
    try {
      if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
      fs.writeFileSync(LOGS_FILE, JSON.stringify(systemLogs, null, 2), 'utf-8');
    } catch (err) {
      console.error('Error persisting logs to disk:', err);
    }
  }, 300);
}
...
export function clearAllSessions(): void {
  ...
  addSystemLog('info', 'SYSTEM', 'Admin flushed all session records.');
}
